...
export const googleNewsProvider = {
  ...
  async collect({ window }) {
    ...
    const timeoutMs = normalizedWindow === "24h" ? 6500 : 5000;

    const settled = await Promise.allSettled(
      QUERIES.map((group) => fetchGroup(group, normalizedWindow, timeoutMs))
    );
    ...
    return {
      ...
      articles: dedupeProviderArticles(articles).slice(0, 180),
...
async function fetchGroup(group, window, timeoutMs) {
  ...
  if (!response.ok) throw new Error(`Google News ${group.id} HTTP ${response.status}`);
  ...
  return items.map((item) => {
    ...
    return {
      title: rawTitle.replace(/\s+-\s+[^-]{2,80}$/, "").trim(),
      url: link,
