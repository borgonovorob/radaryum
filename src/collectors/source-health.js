...
export async function recordCollectorRun(env, run) {
  ...
  // Store a collector history row only when something changed,
  // or at least once every two hours as a heartbeat.
  ...
  const previousTime = previous?.finished_at
    ? new Date(previous.finished_at).getTime()
    : 0;

  const changed =
    !previous ||
    previous.status !== run.status ||
    Number(previous.items_found || 0) !== Number(run.itemsFound || 0) ||
    String(previous.error_message || "") !== String(run.errorMessage || "");

  const heartbeatDue =
    !previousTime ||
    Date.now() - previousTime >= RUN_HEARTBEAT_MS;
  ...
  // Keep current health, but avoid rewriting the row when nothing changed.
  await env.DB.prepare(`
    ...
    ON CONFLICT(provider) DO UPDATE SET
      ...
      last_success_at = CASE
        WHEN excluded.status IN ('healthy', 'partial')
