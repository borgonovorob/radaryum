...
async function initializeAuthentication(){try{await waitForClerk();await window.Clerk.load({ui:{ClerkUI:window.__internal_ClerkUICtor}});authReady=true;q("#signInButton").onclick=openSignIn;q("#gateSignInButton").onclick=openSignIn;q("#signUpButton").onclick=openSignUp;q("#gateSignUpButton").onclick=openSignUp;q("#startTrackingButton").onclick=()=>signedIn()?q("#opportunities")?.scrollIntoView({behavior:"smooth"}):openSignUp();window.Clerk.addListener(renderAuthentication);await renderAuthentication();const params=new URLSearchParams(location.search);if(params.get("signup")==="1"&&!signedIn())openSignUp();}catch(e){console.error(e);q("#status").textContent="Authentication unavailable";}}
